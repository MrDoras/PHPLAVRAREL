                          ...
                          @foreach ($order as $item)
                              <tr>
                                <td>{{$item->UserID}}</td>
                                  ...
                                  <td>
                                    @foreach ($cat as $pro)
                                    @if ($item->catID == $pro->catID)
                                        <p class="text-muted mb-0">{{ $pro->catName}}</p>
                                    @endif
                                    @endforeach
                                  ...
                                  <td>
                                    ...
                                    @foreach ($country as $pro)
                                   
                                    @if (($cus->where('UserID', '=', $item->UserID)->first())->CounID == $pro->CounID)
                                        <p class="text-muted mb-0">{{ $pro->CounName}}</p>
                                        @endif
                                    @endforeach
                                    
                                    @foreach ($city as $pro)
                                    @if (($cus->where('UserID', '=', $item->UserID)->first())->CityID == $pro->CityID)                                       
                                     <p class="text-muted mb-0">{{ $pro->CityName}}</p>
                                        @endif
                                    @endforeach
                                    
                                    {{($cus->where('UserID', '=', $item->UserID)->first())->UserAddress}}
                                </td>
                                <td>{{($cus->where('UserID', '=', $item->UserID)->first())->UserPhone}}</td>
                                  <td>{{$item->status}}</td>
                                  <td>
                                  ...
                                  <form action="{{url('admin/order-status-update')}}" method="POST">
                                      ...
                                      <input type="hidden" name="status" value="Order has been confirm">
