...
<html lang="en">
            ...
            <li class="nav-item dropdown d-none d-xl-inline-block user-dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="UserDropdown">
                <div class="dropdown-header text-center">
                  ...
                  <p class="font-weight-light text-muted mb-0"> {{ $data->email }}</p>
                </div>
                <a class="dropdown-item"href="{{ url('admin/index/profile')}}\{{$data->email}}\{{$data->password}}">My Profile <span class="badge badge-pill badge-danger">1</span><i class="dropdown-item-icon ti-dashboard"></i></a>
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <nav class="sidebar sidebar-offcanvas" id="sidebar">
          <ul class="nav">
            ...
            <li class="nav-item">
              <a class="nav-link" href="{{ url('admin/index/' . $data->email . '/' . $data->password) }}">
            ...
            <li class="nav-item">
              ...
              <div class="collapse" id="ui-basic">
                <ul class="nav flex-column sub-menu">
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/product-list/' . $data->email . '/' . $data->password) }}">Product List</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/product-add/' . $data->email . '/' . $data->password) }}">Add New Product</a>
                  </li>
                </ul>
              </div>
            </li>
            ...
            <li class="nav-item">
              ...
              <div class="collapse" id="auth">
                <ul class="nav flex-column sub-menu">
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/category-list/' . $data->email . '/' . $data->password) }}">Category List</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/category-add/' . $data->email . '/' . $data->password) }}">Add New Category</a>
                  </li>
                </ul>
              </div>
            </li>
            <li class="nav-item">
              ...
              <div class="collapse" id="ui-1">
                <ul class="nav flex-column sub-menu">
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/customer-list/' . $data->email . '/' . $data->password) }}">Customer List</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/customer-order/' . $data->email . '/' . $data->password) }}">Customer's Order</a>
                  </li>
                </ul>
              </div>
            </li>
  ...
  <section class="contact-form-wrapper">
          <div class="container mt-3" style="text-align: center; margin-bottom: 20px;">
              <table class="table table-hover">
                <br/>
                <br/>
                <br/>
                <div style="text-align: center;">
                  <a href="{{ url('user/index') }}/{{ $data->email }}/{{$data->password}}" class="btn btn-primary">See Shopping Page As A Customer</a>
                </div>
                <br/>
                  ...
                  <thead>
                      <tr>
                          <th>Total Profit</th>
                          <th>Best Seller</th>
                          <th>Total Customer's Account</th>
                      </tr>
                  </thead>
                  <tbody>
                      <div style="margin-right: 10%; margin-bottom:20px; float: right;">
                        @csrf
                        <?php $total = 0; $quantity=0;$name="null";$Bestseller=0;?>
                        @foreach ($order as $item)
                          <tr>
                            @if ($item->status == "Order has been confirm")
                              <?php $total += $item->productPrice;?>
                              @foreach ($order as $pro)
                                @if ($item->productName ==$pro->productName)
                                <?php $quantity += $pro->quantity;?>
                                @endif
                                @if ($Bestseller<=$quantity)
                                <?php $name = $pro->productName;?>
                                @endif
                              @endforeach
                            @endif
                          </tr>
                        ...
                        <td>{{$total}}$</td>
                        <td>{{$name}}</td>
                        <td>{{ count($cus) }} Customer
                          <br/>
                          <br/>
                          <a href="{{url('admin/customer-list')}}\{{$data->email}}\{{$data->password}}" class="btn btn-primary">See Customer List</a>
                        </td>
                      </div>
