                        ...
                        <tbody>
                          ...
                          @if($pro->userType == 0)
                              <tr>
                                  ...
                                  @if ($item->CounID==$pro->CounID)
                                  <td>{{$item->CounName}}</td>
                                  @endif
                                @endforeach
                                  @foreach ($city as $item)
                                    @if ($item->CityID==$pro->CityID)
                                    <td>{{$item->CityName}}</td>
                                    @endif
                                  @endforeach
                                  <td>{{$pro->UserAddress}}</td>
                                  <td>
                                      <a href="{{url('admin/customer-edit/' . $user->email . '/' . $user->password . '/' . $pro->UserID )}}" title="Edit this product"><i class="bi bi-pencil-fill"></i></a> &nbsp;
                                      <a href="{{url('admin/customer-delete/' . $pro->UserID)}}" title="Delete this product" onclick="return confirm('Are you sure you want to delete this product?');"><i class="bi bi-x-square"></i></a> &nbsp;
                                  </td>
                              </tr>
                          @endif
                      @endforeach
